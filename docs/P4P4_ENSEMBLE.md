# PHASE 4 PROMPT 4 — Ensemble (Stacking) Predictor

## CONTEXT

This prompt creates `ml/ensemble_predictor.py` — the stacking meta-learner that
combines XGBoost and TFT predictions into a final forward return prediction.

Architecture Section 7 defines the ensemble structure:
```
Raw Data
    ├── Snapshot Features → XGBoost → predicted_return_xgb
    ├── Sequence Data     → TFT     → predicted_return_tft
    └── Meta-Learner (stacking)     → final_predicted_return
        (learns optimal weighting of XGB vs TFT)
```

The meta-learner is trained AFTER both XGBoost and TFT are individually trained.
It is given [xgb_prediction, tft_prediction] as input features and learns the
optimal linear combination that minimizes out-of-sample MAE on the training data.

## DESIGN DECISIONS (read before coding)

**Meta-learner choice: Ridge regression**
Ridge is the correct choice for stacking two correlated signals:
- Linear combination is interpretable (coefficient tells you which model dominates)
- Ridge regularization prevents overfitting when inputs are correlated
- With only 2 input features, there is no risk of underfitting
- No hyperparameter search needed — a small alpha (0.1) is sufficient
- Alternative (XGBoost meta-learner) would overfit badly with only 2 features

**Training data for the meta-learner:**
The meta-learner trains on out-of-fold (OOF) predictions from both models.
This is the correct stacking procedure — using in-sample predictions would
cause the meta-learner to overfit to whichever model memorizes training data.

However: generating true OOF predictions requires rerunning CV for both models
which is expensive. For this implementation, we use the full training dataset
predictions from the already-trained models. This is a known approximation —
it slightly favors XGBoost (which overfits less on tabular data) but is
practical for production use.

**save format:**
Single `.joblib` file containing:
```python
{
    "meta_learner": fitted Ridge object,
    "xgb_model_path": str,       # path to XGBoost .joblib
    "tft_model_dir": str,        # path to TFT directory
    "feature_names": list[str],  # shared feature list (73 for swing)
    "encoder_length": int,       # from TFT (60)
    "xgb_weight": float,         # Ridge coef[0] (informational)
    "tft_weight": float,         # Ridge coef[1] (informational)
}
```

**predict() contract:**
`predict(features, sequence)` must accept BOTH arguments since strategy code
calls `predictor.predict(latest_features)` (no sequence at live trading time
unless we add the sequence window retrieval). 

For now: if `sequence` is None or too short, predict falls back to XGBoost only.
This is the correct degraded mode — live trading can run even if the sequence
window isn't available. The ensemble activates when full sequence data is present.

**predict_batch() for meta-learner training:**
EnsemblePredictor.train_meta_learner() is a method that:
1. Loads the XGBoost and TFT predictors
2. Gets XGBoost predictions on the daily-subsampled dataset
3. Gets TFT predictions on the same rows (using TFTTrainer.predict_dataset)
4. Fits Ridge on [xgb_preds, tft_preds] → actuals
5. Saves the ensemble file

## TASK

Create ONE new file: `options-bot/ml/ensemble_predictor.py`
...
# Read both sub-predictors to understand load/predict interface
cat ml/xgboost_predictor.py
cat ml/tft_predictor.py

# Understand predict_dataset from TFT trainer
grep -n "def predict_dataset" ml/tft_trainer.py

# Understand daily subsample pattern used by XGBoost trainer
grep -n "_subsample_daily\|_calculate_target\|_compute_all_features" ml/trainer.py | head -20

# Check config constants
python -c "from config import MODELS_DIR, DB_PATH; print(MODELS_DIR, DB_PATH)"

# Verify 73 features after fix
python -c "
from ml.tft_trainer import _get_feature_names
names = _get_feature_names('swing')
print(f'Swing features: {len(names)}')  # expect 73
"
```

---

## FILE: `options-bot/ml/ensemble_predictor.py`
...
class EnsemblePredictor(ModelPredictor):
    ...
    def train_meta_learner(
        ...
        try:
            ...
            # Align by index (positional index into featured_df)
            # daily_df index corresponds to rows of featured_df after dropna
            # tft_preds_series index corresponds to bars in seq_df (which mirrors featured_df)
...
# 1. File exists
echo "=== File check ==="
[ -f "ml/ensemble_predictor.py" ] && echo "  OK  ml/ensemble_predictor.py" || echo "  MISSING"
...
print('  OK  EnsemblePredictor imports cleanly')
"

# 3. Interface compliance
echo ""
echo "=== ModelPredictor interface compliance ==="
python -c "
from ml.predictor import ModelPredictor
from ml.ensemble_predictor import EnsemblePredictor

assert issubclass(EnsemblePredictor, ModelPredictor), 'Must extend ModelPredictor'
print('  OK  EnsemblePredictor is subclass of ModelPredictor')

e = EnsemblePredictor()
required = ['predict', 'predict_batch', 'get_feature_names', 'get_feature_importance']
for m in required:
    assert hasattr(e, m) and callable(getattr(e, m)), f'Missing {m}()'
    print(f'  OK  {m}()')
"

# 4. Unloaded state — no crash, no model needed
echo ""
echo "=== Unloaded state ==="
python -c "
from ml.ensemble_predictor import EnsemblePredictor
...
e = EnsemblePredictor()
print(f'  get_feature_names(): {e.get_feature_names()} (expect [])')
print(f'  get_feature_importance(): {e.get_feature_importance()} (expect {{}})')
assert e.get_feature_names() == []
assert e.get_feature_importance() == {}
try:
    import pandas as pd
    e.predict({})
    print('  FAIL — should raise RuntimeError')
except RuntimeError as ex:
    print(f'  OK  predict() raises RuntimeError: {ex}')
print('  Unloaded state PASS')
"

# 5. Degraded mode: TFT unavailable → XGBoost only
echo ""
echo "=== Degraded mode (sequence=None uses XGBoost only) ==="
python -c "
import numpy as np
...
# Wire up mock sub-models
e._xgb = MagicMock()
e._xgb.predict.return_value = 2.5
e._tft = MagicMock()
e._meta_learner = Ridge(alpha=0.1)
e._meta_learner.coef_ = np.array([0.6, 0.4])
e._meta_learner.intercept_ = 0.0
# Fit on dummy data so predict() works
e._meta_learner.fit(np.array([[1,1],[2,2],[3,3]]), np.array([1,2,3]))
e._feature_names = ['f1']

# Test with sequence=None → XGBoost only
result = e.predict({'f1': 1.0}, sequence=None)
print(f'  Degraded mode result: {result} (expect 2.5 from XGBoost mock)')
assert result == 2.5, f'Expected 2.5, got {result}'
print('  Degraded mode PASS')
"

# 6. Full ensemble path: both models available
echo ""
echo "=== Full ensemble path (both models mocked) ==="
python -c "
import numpy as np
import pandas as pd
from unittest.mock import MagicMock
...
e._xgb.predict.return_value = 2.5
e._tft = MagicMock()
e._tft.predict.return_value = 1.8
e._encoder_length = ENCODER_LENGTH
e._feature_names = ['f1']

# Fit meta-learner on synthetic data
ridge = Ridge(alpha=0.1)
ridge.fit(np.array([[2.5, 1.8], [1.0, 0.5], [3.0, 2.5]]), np.array([2.2, 0.8, 2.8]))
e._meta_learner = ridge

# Provide a sequence long enough for TFT
seq = pd.DataFrame(np.random.randn(ENCODER_LENGTH, 1), columns=['f1'])
result = e.predict({'f1': 1.0}, sequence=seq)
print(f'  Full ensemble result: {result:.4f}')
print(f'  XGBoost was called: {e._xgb.predict.called}')
print(f'  TFT was called: {e._tft.predict.called}')
assert e._xgb.predict.called, 'XGBoost must be called'
assert e._tft.predict.called, 'TFT must be called'
assert isinstance(result, float), 'Result must be float'
print('  Full ensemble path PASS')
"

# 7. Combined feature importance (weighted blend)
echo ""
echo "=== Combined feature importance ==="
python -c "
import numpy as np
from unittest.mock import MagicMock
from ml.ensemble_predictor import EnsemblePredictor
...
e = EnsemblePredictor()
e._xgb = MagicMock()
e._xgb.get_feature_importance.return_value = {'rsi_14': 0.3, 'ret_1d': 0.7}
e._tft = MagicMock()
e._tft.get_feature_importance.return_value = {'rsi_14': 0.5, 'ret_1d': 0.3, 'atm_iv': 0.2}
e._xgb_weight = 0.6
e._tft_weight = 0.4
e._feature_names = ['rsi_14', 'ret_1d', 'atm_iv']

imp = e.get_feature_importance()
print(f'  Combined importance: {imp}')
assert 'rsi_14' in imp
assert 'ret_1d' in imp
assert 'atm_iv' in imp
# rsi_14: 0.6*0.3 + 0.4*0.5 = 0.18 + 0.20 = 0.38
assert abs(imp['rsi_14'] - 0.38) < 0.01, f'Expected 0.38, got {imp[\"rsi_14\"]}'
print('  Combined importance PASS')
"

# 8. Save/load round-trip with real Ridge model
echo ""
echo "=== Save/load round-trip ==="
python -c "
import numpy as np
import tempfile, os
from pathlib import Path
...
# Build ensemble with real Ridge
e = EnsemblePredictor()
ridge = Ridge(alpha=0.1)
ridge.fit(np.array([[1,2],[3,4],[5,6]]), np.array([1.5, 3.5, 5.5]))
e._meta_learner = ridge
e._xgb_model_path = '/fake/xgb.joblib'
e._tft_model_dir = '/fake/tft_dir'
e._feature_names = ['feat_a', 'feat_b']
e._encoder_length = 60
e._xgb_weight = 0.0  # will be replaced by save()
e._tft_weight = 0.0

with tempfile.TemporaryDirectory() as tmpdir:
    save_path = os.path.join(tmpdir, 'test_ensemble.joblib')
    e.save(save_path)
    print(f'  Saved to {save_path} ({os.path.getsize(save_path)} bytes)')

    # Load back (without actually loading sub-models)
    import joblib
    data = joblib.load(save_path)
    print(f'  Keys in saved file: {list(data.keys())}')
    assert 'meta_learner' in data
    assert 'xgb_model_path' in data
    assert 'tft_model_dir' in data
    assert 'feature_names' in data
    assert 'xgb_weight' in data
    assert 'tft_weight' in data
    assert data['feature_names'] == ['feat_a', 'feat_b']
    coef = data['meta_learner'].coef_
    print(f'  Ridge coef: {coef}')
    assert len(coef) == 2, 'Ridge must have 2 coefficients'
...
- `ml/ensemble_predictor.py` exists and imports without error
- `EnsemblePredictor` is a concrete subclass of `ModelPredictor`
- All 4 abstract methods implemented: `predict`, `predict_batch`, `get_feature_names`, `get_feature_importance`
- Unloaded state: `get_feature_names()` returns `[]`, `get_feature_importance()` returns `{}`, `predict()` raises `RuntimeError`
- Degraded mode: `predict(features, sequence=None)` returns XGBoost prediction without error
- Full mode: `predict(features, sequence)` calls both sub-models and passes results to Ridge meta-learner
- `get_feature_importance()` returns weighted blend of XGBoost + TFT importances
- Save produces a `.joblib` file containing all required keys including `meta_learner`, `xgb_model_path`, `tft_model_dir`, `feature_names`, `xgb_weight`, `tft_weight`
- Short sequence (< encoder_length): falls back to XGBoost only, no crash

## FAILURE GUIDE

- **`ImportError: cannot import name '_subsample_daily' from ml.trainer`**: This function exists in `trainer.py` — verify it is not prefixed with double underscore (`__`) which would make it private. It should be `_subsample_daily` (single underscore).
- **`ImportError: cannot import name '_calculate_target' from ml.trainer`**: Same — single underscore functions. If they are not importable, copy the logic directly into `ensemble_predictor.py`'s `train_meta_learner()` rather than importing.
- **`IndexError` in combined importance when TFT feature set differs from XGBoost**: The `get_feature_importance()` uses `set(xgb_imp) | set(tft_imp)` to union all features, with `.get(feat, 0.0)` for missing ones. This should handle any mismatch gracefully.
- **`common_idx` is empty**: The TFT `predict_dataset()` returns predictions indexed to 5-min bar positions (starting at `ENCODER_LENGTH - 1`). The daily_df index corresponds to specific bar positions from `featured_df` after `_subsample_daily()`. If the index types mismatch (e.g., RangeIndex vs DatetimeIndex), do `daily_df = daily_df.reset_index(drop=True)` and `featured_df = featured_df.reset_index(drop=True)` before computing `featured_df["_target"]`.
- **Ridge coef has wrong shape**: Ridge with 2 input features always produces `coef_` of shape `(2,)`. If it shows shape `(1, 2)`, access with `coef_[0]` instead of `coef_`.
- **Mock-based tests fail on actual load test**: The save/load round-trip test loads with `joblib.load()` directly (not `EnsemblePredictor.load()`) to avoid the sub-model load which would need real files. This is intentional — structural correctness is verified without needing trained models on disk.

## DO NOT

- Do NOT store the XGBoost or TFT model objects inside the ensemble `.joblib` — store only their file paths
- Do NOT run grid search or hyperparameter tuning on the Ridge — `alpha=0.1` is correct and sufficient
- Do NOT modify `ml/xgboost_predictor.py`, `ml/tft_predictor.py`, or `ml/tft_trainer.py`
- Do NOT modify `ml/trainer.py` — import `_subsample_daily` and `_calculate_target` from it
- Do NOT add a `predict_batch()` that secretly calls both sub-models with sequences — the method docstring correctly explains it is XGBoost-only for interface compliance
- Do NOT add any new dependencies to `requirements.txt` — Ridge is in scikit-learn which is already installed
